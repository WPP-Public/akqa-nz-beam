...
class GenDeploy extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		if ( file_exists( 'deploy.json' ) && !$input->getOption('replace') ) {
			...
			$output->writeln( "<error>Error: deploy.json file already exists in this directory, use '-r (or --replace)' flag to overwrite file</error>" );
			exit;
		...
		if ( $input->getArgument('client_code') ) {
			...
			if (!$dialog->askConfirmation(
					$output,
					"<question>You haven't sepcified a client code, would you like to generate a default deploy.json file? (y,n)</question>",
					false
				)) {
				exit;
			}
		...
		// Save template
		file_put_contents('deploy.json', str_replace('\/', '/', $this->json_format( json_encode( $json_deploy_template ) ) ) );

		$output->writeln( "<info>Success: deploy.json file saved to ". getcwd() . "/deploy.json - be sure to check the file before using it</info>" );
